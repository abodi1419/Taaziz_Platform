    ...
    <div class="container">
        <div class="card">
            ...
            <div class="card-header">
                ...
                @php
                    if(\Illuminate\Support\Facades\Auth::user()->likes()->where('article_id','=',$article->id)->first()){
                        $liked = true;
                    }
                    else{
                        $liked = false;
                ...
                @endphp
                <a id="like-btn" href="{{asset('/like/'.$article->id)}}"  type="button" data-article_id="{{$article->id}}" class="btn @if($liked) btn-success @else btn-danger @endif">
    ...
    <div id="comments" class="container mt-4">
        ...
        @forelse($article->comments as $comment)
            ...
            <div class="card-footer  mb-3">
                <p>{{__('Author')}}: {{$comment->user->name. ' Date :'.date_format($comment->created_at,'d/m/Y')}}</p> {{-- --}}
